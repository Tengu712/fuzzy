...
pub fn insert_variable_definition(maps: &mut FunctionMap, ty: &str) {
    ...
    map.insert("->".to_string(), Function::Builtin(define_mutable_variable));
    map.insert(
        "=>".to_string(),
        Function::Builtin(define_immutable_variable),
    );
}
...
define_make_variable!(define_mutable_variable, "->", true);
define_make_variable!(define_immutable_variable, "=>", false);
